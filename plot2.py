...
COL_5_NAME = "loop bw (hz)"
# ----------------------------

# Create 2 vertically stacked subplots that share the X axis
fig, (ax_top_left, ax_bot_left, ax_bw) = plt.subplots(3, 1, sharex=True)
...
for ax in (ax_top_left, ax_top_right, ax_bot_left, ax_bot_right, ax_bw):
...
def update_plot(frame):
    ...
    for line in tail_lines:
        columns = line.split()
        if len(columns) > max(COL_1_IDX, COL_2_IDX,
                              COL_3_IDX, COL_4_IDX, COL_5_IDX):
            ...
            y5_data.append(float(columns[COL_5_IDX]))
